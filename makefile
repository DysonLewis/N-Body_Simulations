...
TARGET_ACCEL := $(BUILD_DIR)/accel$(EXT_SUFFIX)
TARGET_SIM := $(BUILD_DIR)/simulator$(EXT_SUFFIX)
TARGET_ACCEL_GPU := $(BUILD_DIR)/accel_gpu$(EXT_SUFFIX)
TARGET_SIM_GPU := $(BUILD_DIR)/simulator_gpu$(EXT_SUFFIX)

GPU_CUDA_OBJECT := $(BUILD_DIR)/accel_gpu.cuda.o
GPU_HOST_OBJECT := $(BUILD_DIR)/simulator_gpu.host.o
...
$(TARGET_ACCEL): accel.cpp | $(BUILD_DIR)
	$(CXX) $(CXXFLAGS_CPU) $(INCLUDES) $(CPU_LDFLAGS) -o $@ $<

$(TARGET_SIM): simulator.cpp | $(BUILD_DIR)
	$(CXX) $(CXXFLAGS_CPU) $(INCLUDES) $(CPU_LDFLAGS) -o $@ $<

$(GPU_CUDA_OBJECT): accel_gpu.cu accel_gpu_api.h | $(BUILD_DIR)
	$(NVCC) $(NVCCFLAGS) $(INCLUDES) -dc -o $@ $<

$(GPU_HOST_OBJECT): simulator_gpu.cpp accel_gpu_api.h | $(BUILD_DIR)
...
clean:
	...
	rm -rf __pycache__ $(BUILD_DIR)/__pycache__
	rm -f *.pyc
